...
describe('The searchEmployee function', () => {
  ...
  test('returns correctly when id and detail are valids', () => {
    expect(searchEmployee('8579-6', 'firstName')).toMatch('Ana');
    expect(searchEmployee('5569-4', 'lastName')).toMatch('Jobs');
    expect(searchEmployee('1256-4', 'specialities')).toEqual(['Hooks', 'Context API', 'Tailwind CSS']);
  });
  test('returns an error message when id or/and detail are invalids', () => {
    expect(searchEmployee('XXX', 'lastName')).toMatch('ID não identificada');
    expect(searchEmployee('5569-4', 'XXX')).toMatch('Informação indisponível');
    expect(searchEmployee('XXX', 'YYY')).toMatch('ID não identificada');
  });
});